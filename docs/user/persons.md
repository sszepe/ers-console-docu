...
Open **Persons** from the top navigation. The list shows display name, key identifiers, import source, status, and last-updated date. Filter controls:

- **Search box** — filters by name, ORCID, GND ID, OpenAlex ID, email, or registry ID
- **Status filter** — show only active or inactive records
- **Import source filter** — show records from a specific external system
...
| **Scopus Author ID** | Numeric Scopus ID |
...
Use the **Names** inline to add variant name forms (maiden name, transliteration, display name in another language). Each name record carries:
...
When a person was imported from an external source, the **Import provenance** section (collapsed by default) shows:

| Field | Meaning |
|---|---|
| `imported_from` | Source system name (`orcid`, `openalex`, `gnd`…) |
...
| `import_payload_hash` | SHA-256 of the last imported payload |

<div class="callout callout-warn">
  <span class="callout-title">Field locking</span>
...
See [Review Workflow](/ers-docs/docs/user/review/) for the full picture.

<div class="page-nav">
  <a href="/ers-docs/docs/user/">← User Guide</a>
